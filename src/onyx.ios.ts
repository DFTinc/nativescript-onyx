  ...
  public initAndPresentiOSOnyx(
    ...
    onyxPluginSuccessCallback: OnyxPluginSuccessCallback,
      ...
      if (!this.isSimulator) {
        ...
        onyxVC.delegate = OnyxViewControllerDelegateImpl.new().initWithOwnerAndCallbacks(new WeakRef(this));
        ...
        onyxConfigBuilder = onyxConfigBuilder.setOnyxCallback()((configuredOnyx: Onyx): void => {
          console.log("OnyxPluginiOS: config build ok CALLBACK");
          if (!currentViewController) console.log('OnyxPluginiOS: WARNING: currentViewController is null.');
          configuredOnyx.capture(currentViewController);
          if (this.onyxPluginCallback) {
            this.onyxPluginCallback('configured OK!');
          } else {
            console.log('OnyxPluginiOS: WARNING: configure callback is not set..');
          }
        });
        ...
        onyxConfigBuilder = onyxConfigBuilder.setSuccessCallback()((onyxResult: OnyxResult): void => {
          ...
          console.log("OnyxPluginiOS: --03");


        });
        console.log("OnyxPluginiOS: --04");
...
class OnyxViewControllerDelegateImpl extends NSObject implements OnyxViewControllerDelegate {
  ...
  private owner: WeakRef<OnyxPlugin>;
  private options?: any;

  static new(): OnyxViewControllerDelegateImpl {
      if (OnyxViewControllerDelegateImpl.ObjCProtocols.length === 0 && typeof (OnyxViewControllerDelegate) !== "undefined") {
        OnyxViewControllerDelegateImpl.ObjCProtocols.push(OnyxViewControllerDelegate);
          console.log("OnyxPluginiOS: really hooking the delegate..");
      }
      return <OnyxViewControllerDelegateImpl>super.new();
    }

  public initWithOwnerAndCallbacks(owner: WeakRef<OnyxPlugin>, options?: any): OnyxViewControllerDelegateImpl {
      
      console.log("AssaAbloyiOS: hooking the callback functions..");
  ...
  /* THE CALLBACKS */
	...
	*/
    ...
    if (this.owner.get().onyxPluginErrorCallback) {
      this.owner.get().onyxPluginErrorCallback(error.description);
    }
  }
	OnyxOnCancel(controller: OnyxViewController, error: NSError):  void {
    console.log("OnyxPlugin-: onCancel:" + error);
    if (this.owner.get().onyxPluginErrorCallback) {
      this.owner.get().onyxPluginErrorCallback(error.description);
    }
  }
